...
def get_text():
    ...
    return my_code.decode('GBK')


def set_text(aString):
    wincld.OpenClipboard()
    wincld.EmptyClipboard()
    wincld.SetClipboardData(win32con.CF_TEXT, aString)
    wincld.CloseClipboard()
...
# 对比图片并点击
def computer_matchImg_doubleClick(myScreencap, mypng):
    '''对比图片并点击'''
    if _matchImg(myScreencap, mypng) is not None:
        print("-------------点击按钮！" + mypng + str(
            _matchImg(myScreencap, mypng)['result'][0]) + ',' + str(
        ...
        myx = str(_matchImg(myScreencap, mypng)['result'][0])
        myy = str(_matchImg(myScreencap, mypng)['result'][1])
        computer_double_click(int(float(myx)), int(float(myy)))
        # time.sleep(3)
        print("-------------结束点击按钮。")
